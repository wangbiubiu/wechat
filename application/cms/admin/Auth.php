...
use app\cms\model\Auth as AuthModel;
...
class Auth extends Admin
{
    public function yes(){
        $id=input('id');
        $res=Db::name("cmsAuth")->where(['id'=>$id])->update(['status'=>1]);
        if($res){
    ...
    public function no(){
        $id=input('id');
        $res=Db::name("cmsAuth")->where(['id'=>$id])->update(['status'=>2]);
        if($res){
            $this->success('处理成功', 'index');
        }
    }
    ...
    public function index()
    {
        ...
        // 数据列表
        $data_list = AuthModel::where($map)->order($order)->paginate();
        // 使用ZBuilder快速创建数据表格
        return ZBuilder::make('table')
            ->setSearch(['name' => '审核人']) // 设置搜索框
            ->addColumns([ // 批量添加数据列
                ['id', 'ID'],
                           ['name', '姓名'],
                           ['add', '地址'],
...
//            }
            ...
            if ($Auth = AuthModel::create($data)) {
                // 记录行为
                action_log('Auth_add', 'cms_Auth', $Auth['id'], UID, $data['title']);
...
//            ->setPageTips('如果出现无法添加的情况，可能由于浏览器将本页面当成了广告，请尝试关闭浏览器的广告过滤功能再试。', 'warning')
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $advert_name = AuthModel::where('id', 'in', $ids)->column('title');
    ...
    public function quickEdit($record = [])
    {
        ...
        $advert  = AuthModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $advert . ')，新值：(' . $value . ')';
        return parent::quickEdit(['Auth_edit', 'cms_Auth', $id, UID, $details]);
    }
}