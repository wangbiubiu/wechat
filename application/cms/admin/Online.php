...
class Online extends Admin
{
    public function yes(){
        $id=input('id');
        $res=Db::name("cms_online")->where(['id'=>$id])->update(['status'=>1]);
        if($res){
    ...
    public function no(){
        $id=input('id');
        $res=Db::name("cms_online")->where(['id'=>$id])->update(['status'=>2]);
        if($res){
            $this->success('处理成功', 'index');
        }
    }
    public function ok(){
        $id=input('id');
        $res=Db::name("cms_online")->where(['id'=>$id])->update(['status'=>3]);
        if($res){
            $this->success('处理成功', 'index');
        }
    }
...
//            ->addTopButtons('add,enable,disable,delete') // 批量添加顶部按钮
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            // 验证
            $result = $this->validate($data, 'Online');
...
//            }
            ...
            if ($Online = OnlineModel::create($data)) {
                // 记录行为
                action_log('Online_add', 'cms_Online', $Online['id'], UID, $data['title']);
...
//            ->setPageTips('如果出现无法添加的情况，可能由于浏览器将本页面当成了广告，请尝试关闭浏览器的广告过滤功能再试。', 'warning')
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        $advert_name = OnlineModel::where('id', 'in', $ids)->column('title');
    ...
    public function quickEdit($record = [])
    {
        ...
        $advert  = OnlineModel::where('id', $id)->value($field);
        $details = '字段(' . $field . ')，原值(' . $advert . ')，新值：(' . $value . ')';
        return parent::quickEdit(['Online_edit', 'cms_Online', $id, UID, $details]);
    }
}